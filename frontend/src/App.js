		...
		return (
			<div style={{textAlign: 'center'}}>
				<ul className="nav nav-tabs" role="tablist" data-tabs="tabs" style={{textAlign: 'center', display: 'inline-block'}}>
					<li><a data-toggle="tab" ref="tc" style={{fontSize: '45px'}} onClick={() => this.changeView('best')}>Best General</a></li>
...
class Graph extends React.Component {
	...
	hideGraph() {
		...
		setTimeout(function(){
			document.getElementsByClassName('highcharts-container')[0].style.height = '0px';
		}, 1000);
	}
	render() {
		if(this.props.view == 'visible') {
			this.refs.gc.style.opacity = '1';
			this.refs.gc.style.height = '400px'
			this.refs.c.style.height = '500px'
			...
			document.getElementsByClassName('highcharts-container')[0].style.height = '400px';
		}
		return (
			<div id="wrapper" ref="c" style={{visibility: this.props.view, height: '0px'}}>
			 <h2 ref="tit" id="graph-title">{this.props.children} </h2>
			 <hr ref="bar1" />
			 <div ref="gc" id="graph-container" style={{width: '85%', marginLeft: '7.5%', opacity: '0', marginBottom: '15px'}}></div>
